  ...
  methods: {
    checkAll(val){
     ...
     if(val.length>0){
        val.forEach( (item,index) => {
          ...
          this.getMultipleSelections(item)
          //console.log('全选1111')
        })
    ...
    getMultipleSelections(row) {
      ...
      let setSelection = (data) => {
        data.map((item)=>{
          ...
          if(item.children!=null&&item.children!='null'&&item.children!=''){
      ...
      if(row.children!=null&&row.children!=''&&row.children.length>0){
    ...
    selectionChangeNative(selection,row){
      ...
      if (!ifCheck) {
        ...
        console.log(Object.prototype.toString.call(ids[0]),'Object.prototype.toString.call(ids[0])')
        ...
        //console.log(this.getMultipleSelections(row),"xia")
        this.getMultipleSelections(row).map((item,index)=>{
