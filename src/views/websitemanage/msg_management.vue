...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        userName: [{ required: true, message: '留言人必填', trigger: 'blur' }],
        title: [{ type: 'string', required: true, trigger: 'blur', validator: validateTitle }],
        content: [{ type: 'string', trigger: 'blur', validator: validateContent }],
        auditStatus: [{ required: true, message: '状态必选', trigger: 'blur' }]
      },
      multipleSelection:[]
    }
  },
    ...
    massSettingSubmit(){
      ...
      if (this.massQuery.auditStatus === null) {
        ...
        massReview(this.massQuery).then(res => {
          operationMsg.apply(this,[res.data,`批量审核${this.multipleSelection.length}项`])
    ...
    massReplySubmit(){
      ...
      if (this.replyContent === null || this.replyContent === '' ) {
        ...
        massReply(this.massQuery).then(res => {
          operationMsg.apply(this,[res.data,'批量留言回复'])
        })
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addMessage(newTemp).then(res => {
            operationMsg.apply(this,[res.data,'创建留言'])
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateMessage(tempData).then(res => {
            operationMsg.apply(this,[res.data,'留言更新'])
            this.dialogFormVisible = false
            document.getElementsByClassName('form-submit').disabled = false
          })
        }
