  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button v-if="dialogStatus=='create'" @click="createData" :disabled="sending">保存</el-button>
        <el-button v-else @click="updateData" :disabled="sending">保存</el-button>
...
<script>
import { getData, changeStatus, massDeleteAPI, updateAdPos, createAdPos } from '@/api/website-management/adPos'
...
export default {
  ...
  data() {
    ...
    const validateValue = (rule, value, callback) => {
      let reg = /^[a-zA-Z0-9]\w{0,29}$/;
      if ( !reg.test(value) ){
        callback(new Error('请输入英文或数字开头，英文、数字、_组成的最多30个字符的广告位代码'))
      } else {
        callback()
      }
    };
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createAdPos(newTemp).then(res => {
            console.log(res)
            operationMsg.apply(this,[res.data,'添加广告位'])
            this.sending = false
