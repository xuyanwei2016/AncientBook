<template>
  <div class="app-container">
    ...
    <el-table stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row @selection-change="handleSelectionChange" @current-change="handleCurChange" ref="multipleTable">
      ...
      <el-table-column label="所属栏目" min-width="80" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="发布时间" min-width="80" align="center">
        <template slot-scope="scope">
          <span>{{scope.row.releaserTime}}</span>
        </template>
      </el-table-column>
      <el-table-column label="上下架状态" min-width="60" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" v-if='dialogFormVisible'>
      ...
      <div slot="footer" class="dialog-footer" v-if="dialogStatus!=='view'">
        <el-button @click="submitForm" :loading="sending">确 定</el-button>
  ...
  methods: {
    ...
    setTopStatus(status){
      ...
      updateTopAPI(this.batchData).then(res => {
        status === 1 ? operationMsg.apply(this,[res.data,`置顶${this.batchData.ids.length}项`]) : operationMsg.apply(this,[res.data,`取消置顶${this.batchData.ids.length}项`])
      })
    },
    deleteItem(idArr){
      confirm.apply(this,['确定删除选择的条目吗?']).then(() => {
        batchingDeleteAPI(idArr).then(res => {
          operationMsg.apply(this,[res.data,`删除${idArr.length}项`])
        })
      })
    },
