  ...
  <div class="app-container">
    ...
    <el-form inline>
      ...
      <el-form-item label="上架状态">
        <el-select placeholder="上架状态" v-model="listQuery.showState">
    ...
    <el-table stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row @selection-change="handleSelectionChange" @current-change="handleCurChange" ref="multipleTable">
      ...
      <el-table-column label="上架状态" min-width="80" align="center">
        <template slot-scope="scope">
          <span>{{scope.row.showState ? '已上架' : '已下架'}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="dataForm" label-position="right" label-width="110px" style="margin-right:30px;">
        ...
        <el-form-item label="上下架状态：" prop="showState">
    ...
    batchAudit(){
      confirm.apply(this,['确定审核通过已选内容吗?']).then(() => {
        batchingAuditAPI(this.batchData.ids).then(res => {
          operationMsg.apply(this,[res.data,`批量审核${this.batchData.ids.length}项`])
        })
      })
    },
    deleteItem(idArr){
    ...
    setStatus(status){
      ...
      if (status === 1) {
        ...
        if (allAudited) {
          confirm.apply(this,['确定上架已选内容吗？']).then(() => {
            batchingListing(this.batchData).then(res => {
              operationMsg.apply(this,[res.data,`上架${this.batchData.ids.length}项`])
            })
          })
        } else {
        ...
        confirm.apply(this,['确定下架已选内容吗？']).then(() => {
          batchingListing(this.batchData).then(res => {
            operationMsg.apply(this,[res.data,`下架${this.batchData.ids.length}项`])
          })
        })
      }
    },
