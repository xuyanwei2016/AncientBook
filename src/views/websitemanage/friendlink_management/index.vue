<template>
  <div class="app-container">
    <div class="buttons">
      <router-link :to="{path:`friendlink_management/edit_friendlink`}" v-if="authBtns.filter(i => i.idName=='addbtn').length > 0">
    ...
    <el-form inline class="demo-form-inline">
      <el-form-item label="友情链接标题">
        <el-input @keyup.enter.native="handleFilter" style="width: 400px;" placeholder="友情链接标题" class="filter-item" v-model="listQuery.title" clearable>
        </el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table :key='tableKey' stripe :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row @current-change="handleCurChange" ref="multipleTable" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" label="操作" min-width="40px" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <router-link :to="{path:`friendlink_management/edit_friendlink`,query:{id:scope.row.id}}" v-if="authBtns.filter(i => i.idName=='editbtn').length > 0">
  ...
  methods: {
    ...
    massDelete(){
      if(this.multipleSelection.length>0){
        confirm.apply(this,['确定要删除选择的友情链接吗?']).then(() => {
          deleteFriendlink(this.massQuery.ids).then(res => {
            operationMsg.apply(this,[res.data,`批量删除${this.multipleSelection.length}项友情链接`])
          })
        })
      } else {
        this.$message.warning('请先选择需要删除的友情链接')
      }
    },
    singleDelete(id) {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          addFriendlink(this.temp).then(res => {
            operationMsg.apply(this,[res.data,'友情链接创建'])
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          updateFriendlink(tempData).then(res => {
            operationMsg.apply(this,[res.data,'友情链接更新'])
            this.dialogFormVisible = false
          });
        }
      });
