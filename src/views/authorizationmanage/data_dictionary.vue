  ...
  <div class="app-container project-plan">
    ...
    <div class="filter-container">
      ...
      <el-form inline class="form-inline formlist" label-width="82px">
        <el-form-item label="标识" >
          <el-input @keyup.enter.native="handleFilter" clearable v-model="listQuery.code">
          </el-input>
        </el-form-item>
        <el-form-item label="中文键" >
          <el-input @keyup.enter.native="handleFilter" clearable v-model="listQuery.cnkey">
          </el-input>
        </el-form-item>
        <el-form-item label="英文键" >
          <el-input @keyup.enter.native="handleFilter" clearable v-model="listQuery.enkey">
          </el-input>
        </el-form-item>
        <el-form-item label="值" >
          <el-input @keyup.enter.native="handleFilter" clearable v-model="listQuery.value">
          </el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :before-close="cancel">
      ...
      <div slot="footer" class="dialog-footer" style="text-align: center;">
        <el-button type="primary" v-if="dialogStatus=='update'" class="form-submit" @click="updateBtn" >保存</el-button>
        <el-button type="primary" v-else class="form-submit" @click="createBtn">保存</el-button>
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createData(newTemp).then(res => {
            console.log(res)
            operationMsg.apply(this,[res.data,'添加数据'])
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateData(tempData).then(res => {
            console.log(res)
            operationMsg.apply(this,[res.data,'更新数据'])
            this.dialogFormVisible = false
            // document.getElementsByClassName('form-submit').disabled = false
          })
        }
