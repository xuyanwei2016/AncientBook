<template>
  <div class="project-plan app-container ">
    ...
    <el-dialog title="设置权限" :visible.sync="dialogPvVisible" width="30%">
      ...
      <el-tree :data="modules" ref="tree" show-checkbox node-key="id" :default-checked-keys="checkedIds" :props="defaultProps" @check="setChecked">
      </el-tree>
...
import { getMaxSort, getMultipleSelectionIDs, insertBtnResUndis } from '@/filters/getIds'
  ...
  methods: {
    ...
    openRoleList(id){
      ...
      getRoleBtnAPI(id).then(res => {
        if (res.data.code === 0) {
          console.log(res.data.data,"this.checkedIds")
    ...
    moduleSubmit(){
      ...
      if(this.formData.idList.length>0){
        authorityAPI(this.formData).then(res => {
          operationMsg.apply(this,[res.data,'修改角色权限'])
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addAPI(this.temp).then(res => {
            operationMsg.apply(this,[res.data,'角色创建'])
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateAPI(tempData).then(res => {
            operationMsg.apply(this,[res.data,'角色更新'])
            this.sending = false
            res.data.code === 0 ? this.dialogFormVisible = false : true
          })
        }
    ...
    getBtn(){
      getBtnsAPI().then(res => {
        if (res.data.code === 0) {
          ...
          insertBtnResUndis(this.modules, formatData)         
        } else {
