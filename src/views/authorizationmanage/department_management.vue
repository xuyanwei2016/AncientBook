<template>
  <div class="project-plan app-container">
    ...
    <tree-table :data="list" :evalFunc="func" :evalArgs="args" :expandAll="expandAll" border v-loading="listLoading" :par-selection="handleSelectionChange" :getFormatData="getFormatData" label-name="部门名称" first-column="departmentName">
      <el-table-column align="center" label="部门代号" min-width="200">
        <template slot-scope="scope">
          <span>{{scope.row.code}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="介绍" min-width="200">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="30%">
      ...
      <div slot="footer" class="dialog-footer" style="text-align: center;">
        <el-button   type="primary" v-if="dialogStatus=='update'" @click="updateData" :loading="loading">保存</el-button>
    ...
    <el-dialog title="修改模块权限" :visible.sync="dialogPvVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="moduleSubmit" :loading="loading">确定</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        departmentName: [{ required:true, pattern:/^([\w\d]|[\u4e00-\u9fa5]){1,20}$/,trigger: 'blur',message:'请输入20字符以内的汉字、字母和数字' }]
      },
      dialogStatus: '',
      loading: false,
  ...
  methods: {
    ...
    moduleSubmit() {
      ...
      updateModuleAPI(this.form).then(res => {
        operationMsg.apply(this,[res.data,'修改模块权限'])
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateAPI(tempData).then(res => {
            operationMsg.apply(this,[res.data,'部门更新'])
            res.data.code === 0 ? this.dialogFormVisible = false : true
            this.loading = false
          })
        }
