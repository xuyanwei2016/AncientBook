    ...
    <el-aside width="200px" class="tree-aside">
      ...
      <el-main>
        <div class="project-plan">
          <div class="buttons">
            <el-button  @click="handleForm" v-if="authBtns.filter(i => i.idName=='addbtn').length>0">{{authBtns.filter(i => i.idName=='addbtn')[0].name}}<i v-if="authBtns.filter(i => i.idName=='addbtn').length>0 && authBtns.filter(i => i.idName=='addbtn')[0].icon" :class="[authBtns[authBtns.map(i => i.idName).indexOf('addbtn')].icon, 'el-icon--right']"></i></el-button>
            <el-button  @click="massDelete" v-if="authBtns.filter(i => i.idName=='delbtn').length>0">{{authBtns.filter(i => i.idName=='delbtn')[0].name}}<i v-if="authBtns.filter(i => i.idName=='delbtn').length>0 && authBtns.filter(i => i.idName=='delbtn')[0].icon" :class="[authBtns[authBtns.map(i => i.idName).indexOf('delbtn')].icon, 'el-icon--right']"></i></el-button>
          </div>
          ...
          <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="40%">
            ...
            <div slot="footer" class="dialog-footer">
              <el-button @click="formSubmit" :loading="sending" type="primary">保存</el-button>
...
export default {
  ...
  mounted() {
    ...
    console.log(this.authBtns,'65556565')
    this.getModule().then(() => {
      this.getList()
    })
  },
    ...
    getModule(){
      return getModuleAPI().then(res => {
        if (res.data.code === 0) {
          ...
          console.log(JSON.stringify(this.modules[0].children[0]))
