  ...
  <el-container>
    ...
    <el-main>
      <div class="buttons">
        ...
        <!-- 批量绑定防伪码 -->
        <el-button v-if="authBtns.map(i => i.idName).indexOf('setbtn') >= 0" @click="bindISBN">{{authBtns.filter(i => i.idName=='setbtn')[0].name}}<i v-if="authBtns.filter(i => i.idName=='setbtn').length>0 && authBtns.filter(i => i.idName=='setbtn')[0].icon" :class="[authBtns[authBtns.map(i => i.idName).indexOf('setbtn')].icon, 'el-icon--right']"></i></el-button>
      </div>
      ...
      <el-dialog title="批量绑定防伪码" :visible.sync="dialogbindVisible">
        <el-form label-width="130px">
          ...
          <el-form-item label="明码起始">
            <input class="inputstyle" type="number" required v-model="bindISBNdata.minSort">—
            <input class="inputstyle" type="number" v-model="bindISBNdata.maxSort">
          </el-form-item>
        </el-form>
        <div class="el-loading-mask" v-show="isShowBox">
          <div class="el-loading-spinner">
            <svg viewBox="25 25 50 50" class="circular">
              <circle cx="50" cy="50" r="20" fill="none" class="path"></circle>
            </svg>
            <p class="el-loading-text">给我一点时间</p></div>
        </div>
        <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    IsbnSubmit(){
      if(this.bindISBNdata.isbn!=''&&this.bindISBNdata.minSort!==''&&this.bindISBNdata.maxSort!==''&&this.bindISBNdata.minSort<=this.bindISBNdata.maxSort){
        console.log(this.bindISBNdata.minSort)
        this.isShowBox=true;
