    ...
    <el-main>
      ...
      <el-form inline class="filter-form-inline">
        ...
        <el-form-item label="销售状态">
          <el-select v-model="listQuery.saleStatus" style="width:120px;">
            <el-option label="全部" :value="null"></el-option>
        ...
        <el-form-item label="书刊类型">
          <el-select v-model="listQuery.foreignJournalsStatus" style="width:120px;">
            <el-option label="全部" :value="null"></el-option>
            <el-option label="外版书" :value="1"></el-option>
            <el-option label="本版书" :value="0"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="预售状态">
          <el-select v-model="listQuery.presaleStatus" style="width:120px;">
            <el-option label="全部" :value="null"></el-option>
            <el-option label="预售" :value="1"></el-option>
            <el-option label="不预售" :value="0"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="有无附件">
          <el-select v-model="listQuery.fileStatus" style="width:120px;">
            <el-option label="全部" :value="null"></el-option>
        ...
        <el-form-item>
          <el-button class="search" @click="handleFilter" icon="el-icon-search">检索</el-button><el-button @click="resetFilter">重置</el-button>
        </el-form-item>
      </el-form>
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="40%" v-if="dialogFormVisible">
        ...
        <el-form :model="picTemp" ref="massForm" v-else-if="dialogStatus==='relate'" label-width="100px" style='margin-right:20px;'>
          <!-- <el-form-item label="添加图片：" prop="relationPic">
            <el-upload :action="`${domain}/zuul${requestPath.file}${uploadPath}`" list-type="picture-card" :on-remove="handleRemove" :on-preview="handlePicPreview" :on-success="handlePicSuccess" :before-upload="beforePicUpload" :disabled="uploading">
          ...
          <tinymce v-model="picTemp.relationPicture" ></tinymce>
        </el-form>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button v-if="dialogStatus==='presale'" @click="presaleSubmit" :loading="sending">确定</el-button>
          <el-button v-else-if="dialogStatus==='subtitle'" @click="subtitleSubmit" :loading="sending">确定</el-button>
          <el-button v-else-if="dialogStatus==='upload'" @click="uploadValid" :loading="sending">确定</el-button>
          <el-button v-else-if="dialogStatus==='relate'" @click="relateSubmit" :loading="sending">确定</el-button>
          <el-button v-else-if="dialogStatus==='hint'" @click="hintSubmit" :loading="sending">确定</el-button>
  ...
  methods: {
    ...
    relateSubmit(){
      ...
      relatePicAPI(this.picTemp).then(res => {
        operationMsg.apply(this,[res.data,'关联图片'])
        res.data.status ? this.dialogFormVisible = false : true
        this.sending = false
        this.getList()
      })
      ...
      this.$refs['massForm'].validate((valid) => {
        if (valid) {
          ...
          subtitleAPI(this.subtitleData).then(res => {
            operationMsg.apply(this,[res.data, `批量设置${this.multipleSelection.length}项副标题`])
            this.sending = false
            res.data.status ? this.dialogFormVisible = false : true
          })
        }
      ...
      this.$refs['massForm'].validate((valid) => {
        if (valid) {
          ...
          presaleAPI(this.presale).then(res => {
            operationMsg.apply(this,[res.data, `批量设置${this.multipleSelection.length}项预售`])
            this.sending = false
            res.data.status ? this.dialogFormVisible = false : true
          })
        }
