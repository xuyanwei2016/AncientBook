<template>
  <div class="app-container">
    ...
    <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="120px"
             class="double-column-form clearfix">


      <el-form-item label="商品分类：" prop="diytype">
        <el-popover popper-class="category-popover" placement="bottom-start" trigger="click">
          <el-tree :data="navTree" ref="tree" show-checkbox check-strictly node-key="id"
                   :default-checked-keys="checkedKeys" :props="defaultProps" @check="setSelect"></el-tree>
          <el-select v-model="temp.diytype" value-key="diytypeid" multiple @remove-tag="setTree"
      ...
      <el-form-item label="出版社：" prop="book.publisherid">
        <el-select v-model="temp.book.publisherid" style="width:100%;" placeholder="请选择">
          <el-option v-for="publisher in publishers" :key="publisher.publisherid" :label="publisher.name"
                     :value="publisher.publisherid"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="定价：" prop="pbook.price">
        <el-input-number v-model="temp.pbook.price" :precision="2" :min="0" :max="99999"
                         @change="priceChange"></el-input-number>&nbsp; 元
      </el-form-item>
      <el-form-item label="书代号：" prop="pbook.bookNo">
        <el-input v-model="temp.pbook.bookNo" clearable @change="bookNoRepeat" :class="{formInput:bookNoStatus}"></el-input>
        <span class="formInputSpan" v-if="bookNoStatus">书代号重复</span>
      </el-form-item>
      <el-form-item label="折扣：" prop="pbook.discount">
        <el-input-number v-model="temp.pbook.discount" :precision="2" :step="0.01" :min="0" :max="1"
                         @change="discountChange"></el-input-number>
      </el-form-item>
      ...
      <el-form-item label="销售价：" prop="pbook.salePrice">
        <el-input-number v-model="temp.pbook.salePrice" :precision="2" :min="0" :max="temp.pbook.price"
                         @change="salePriceChange"></el-input-number>&nbsp; 元
      </el-form-item>
      ...
      <el-form-item label="出版日期：" prop="publishTime">
        <el-date-picker v-model="temp.book.publishTime" type="date" value-format="yyyy-MM-dd"
      ...
      <el-form-item label="图片集：" prop="picList" class="width100">
        <el-upload :action="`${domain}/zuul${requestPath.file}${uploadPath}`" list-type="picture-card"
                   :file-list="temp.picList" :on-preview="handlePicPreview" :before-upload="beforePicUpload"
                   :on-remove="handleRemove" :on-success="handleSuccess" :disabled="uploading">
      ...
      <el-form-item label="" class="width100">
        <el-tabs v-model="activeTab" type="border-card">
          <el-tab-pane label="商品简介" name="intro">
            <tinymce v-model="temp.pbook.summary"></tinymce>
          </el-tab-pane>
          <el-tab-pane label="详细介绍" name="detail">
            <tinymce v-model="temp.pbook.introduce"></tinymce>
          </el-tab-pane>
        </el-tabs>
      </el-form-item>
      ...
      <el-form-item label="商品视频：" prop="video" class="width100">
        ...
        <el-col :span="12">
          ...
          <el-upload class="video-uploader clearfix" style="z-index: 111"
                     :action="`${domain}/zuul${requestPath.file}${uploadPath}`" :show-file-list="false"
            ...
            <video v-if="temp.pbook.video"
                   :src="`${domain}${requestPath.file}?fileName=${temp.pbook.video}&isOnLine=true`" controls>
              抱歉，您的浏览器不支持内嵌视频，你可以<a :href="`${domain}${requestPath.file}?fileName=${temp.pbook.video}&isOnLine=true`"
      ...
      <el-form-item label="关联图片：" prop="relationPicture" class="width100">
        <!-- <el-upload class="relative-uploader" :action="`${domain}/zuul${requestPath.file}${uploadPath}`" :show-file-list="false" :on-success="handlePicSuccess" :before-upload="beforePicUpload" :disabled="uploading">
        ...
        <tinymce v-model="temp.pbook.relationPicture"></tinymce>
      ...
      <el-form-item label="上传附件：" v-if="temp.pbook.fileStatus===1 || temp.pbook.fileStatus===2">
        <a v-if="temp.pbook.fileStatus===1"
           :href="`${domain}${requestPath.file}?fileName=${temp.pbook.file}&isOnLine=true`"
           style="text-decoration:underline;color:#409EFF;" target="_blank">点击查看</a>
      ...
      <el-form-item label="邮费选择：" prop="pbook.postage">
        <el-select v-model="temp.pbook.postage" style="width:100%;" placeholder="请选择">
          <el-option v-for="express in expresses" :key="express.id" :label="express.name"
                     :value="express.id"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="最后印刷时间：" prop="lastPublishTime">
        <el-date-picker v-model="temp.pbook.lastPublishTime" type="date" value-format="yyyy-MM-dd"
                        clearable></el-date-picker>
      </el-form-item>
      ...
      <el-form-item label="防伪码正版提示语：" prop="pbook.securitycodeGenuine">
        <el-input v-model="temp.pbook.securitycodeGenuine" :autosize="{ minRows: 4 }" type="textarea"
                  placeholder="最多200字" clearable></el-input>
      </el-form-item>
      <el-form-item label="防伪码盗版提示语：" prop="pbook.securitycodePiracy">
        <el-input v-model="temp.pbook.securitycodePiracy" :autosize="{ minRows: 4 }" type="textarea"
                  placeholder="最多200字" clearable></el-input>
      </el-form-item>
    </el-form>
...
<script>
  ...
  import {
    ...
    relatePicAPI,
    bookCheckBookAPI
  } from '@/api/book-infomation/goodsInfo'
  ...
  export default {
        ...
        rules: {
          ...
          pbook: {
            ...
            bookNo: [
              {type: 'string', required: true, trigger: 'change', message: "未填写", whitespace: true},
              {max: 100, message: "最多100个字符", trigger: 'change'},
              /*{  validator: this.bookNoRepeat, trigger: 'change',message:'重复'}*/
            ],
            ...
            basicOrderCount: {
              required: true,
              trigger: 'change',
              pattern: /^[1-9]\d*\.\d*|0\.\d*[1-9]\d*|[1-9]\d*|0$/,
              message: '请输入大于等于零的数'
            },
            roomNo: [
              {type: 'string', trigger: 'change', max: 100, message: '最多100个字符'},
            ...
            securitycodeGenuine: [{type: 'string', trigger: 'change', max: 200, message: '最多200个字符'}, {
              whitespace: true,
              message: '不可只填写空格',
              trigger: 'change'
            }],
            securitycodePiracy: [{type: 'string', trigger: 'change', max: 200, message: '最多200个字符'}, {
              whitespace: true,
              message: '不可只填写空格',
              trigger: 'change'
            }],
          ...
          picList: {type: 'array', message: '请至少上传一张图片'}
        },
        uploadUrl: '',
    ...
    methods: {
        ...
        getInfoAPI(this.$route.query.id).then(res => {
          if (res.data.code === 0) {
            ...
            this.temp.picList = this.temp.cover.map(item => { // 图片集数组
              return {
                url: `${this.domain}${this.requestPath.file}?fileName=${item.url}&isOnLine=true`,
                response: item.url,
                name: item.name
              }
            })
      ...
      /*删除视频*/
      deleteBtn() {
        this.temp.pbook.video = null
      },
      /*判断书代号是不是重复*/
      bookNoRepeat() {
        let itemId=null;
        if(this.dialogStatus === 'update'){//修改
          itemId=this.$route.query.id
        }
        if (this.temp.pbook.bookNo != '') {
          bookCheckBookAPI({
            bookNo: this.temp.pbook.bookNo,
            id: itemId
          }).then(res => {
            this.bookNoStatus = res.data.data;
          })
        }else{
          this.bookNoStatus=false;
        }
