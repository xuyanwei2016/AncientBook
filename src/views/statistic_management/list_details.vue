<template>
  <div class="app-container">
    ...
    <el-table :key='tableKey' stripe :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row>
      ...
      <el-table-column label="订单号" min-width="120" align="center">
        <template slot-scope="scope">
          <router-link class="bookLink" :to="{name: 'order_processing', query: {id: scope.row.id}}">{{scope.row.code}}</router-link>
        </template>
      </el-table-column>
    ...
    <div class="summary-data">总订单数: {{total}}、总商品数量: {{summaryData.totalGoodsNum }}、总销售额: {{summaryData.totalPayment }}、总码洋: {{summaryData.totalOrderPayment}}、总实洋: {{summaryData.totalRealPayment }}、总折扣额: {{summaryData.totalDiscountPrice }}、总读者运费: {{summaryData.totalReaderCarriage }}、总实际运费: {{summaryData.totalRealReaderCarriage}}</div>
...
import { getListAPI, getAllStatusArginAPI, getSummaryAPI } from "@/api/statistic-management/listDetails"
...
export default {
  ...
  mounted() {
    ...
    var currentdate = year + seperator1 + '01' + seperator1 + '01 00:00:00';
    this.listQuery.beginCreateTime=currentdate;
    this.commitTime=[];
    this.commitTime[0] = new Date(currentdate)
    this.commitTime[1] = new Date()
    this.getList()
  },
  methods: {
    exportData() {
    ...
    handleFilter() {
      if (this.commitTime) {
        ...
        if(typeof this.commitTime[0]=='string'){
          ...
          this.listQuery.beginCreateTime = year + seperator1 + '01' + seperator1 + '01 00:00:00';
          this.listQuery.endCreateTime = year + seperator1 + month + seperator1 + day+' '+hour+':'+minutes+':'+seconds
        }
