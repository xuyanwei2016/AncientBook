<template>
  <div class="app-container">
    ...
    <el-table :key='tableKey' stripe :data="list" @sort-change="handleSort" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row>
      ...
      <el-table-column label="总数量" prop="num" sortable="custom" min-width="60" align="center"></el-table-column>
    ...
    <div class="summary-data">
      不包含套餐商品 ———— 总商品数量: {{summaryData.totalNum}}、总码洋: {{summaryData.totalMinistrial}}、总折扣额: {{summaryData.totalDiscount}}、总实洋: {{summaryData.totalRealOcean}}<br>
      <!--包含套餐的统计-->
      总商品数量: {{summaryData.totalNum1 }}、总码洋: {{summaryData.totalMinistrial1 }}、总折扣额: {{summaryData.totalDiscount1 }}、总实洋: {{summaryData.totalRealOcean1}}
...
import { getAllStatusArginAPI } from "@/api/statistic-management/listDetails"
    ...
    exportData() {
      ...
      for (const key in params) {
        if (({}).hasOwnProperty.call(params, key) && params[key]!== null && params[key]!== '' && params[key]!== undefined) {
          ...
          if(key=='orderBy'){
            tempParams.orderByExport = params.orderBy
          }else{
