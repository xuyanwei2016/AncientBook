  ...
  <div class="app-container project-plan"> 
    ...
    <div class="filter-container">
      <div class="buttons">
        ...
        <!-- <el-button v-if="authBtns.map(i => i.idName).indexOf('leadBtn') >= 0" @click="lead">{{authBtns.filter(i => i.idName=='leadBtn')[0].name}}<i v-if="authBtns.filter(i => i.idName=='leadBtn').length>0 && authBtns.filter(i => i.idName=='leadBtn')[0].icon" :class="[authBtns[authBtns.map(i => i.idName).indexOf('leadBtn')].icon, 'el-icon--right']"></i></el-button> -->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="30%">
      <el-form ref="dataForm" :model="temp" label-position="right" label-width="100px" style="margin-right:10px;" :rules="rules">
        <el-form-item v-if="dialogStatus=='createChild'" label="上级标签：" prop="parentName">
          <el-input v-model="parentName" disabled></el-input>
        </el-form-item>
        <el-form-item label="标签内容：" prop="labelContent">
          <el-input v-model="temp.labelContent" style='width:80%;'></el-input>
        </el-form-item>
        <el-form-item label="英文名称：" prop="englishName">
          <el-input v-model="temp.englishName" style='width:80%;'></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer" style="text-align: center;">
        <el-button v-if="dialogStatus=='update'" @click="updateData" :loading="loading" type="primary">保存</el-button>
        <el-button v-else @click="createData" :loading="loading" type="primary">保存</el-button>
...
import { resourceList, createContentAPI, updateContentAPI, batchDelAPI } from '@/api/business/label'
...
export default {
  ...
  data() {
    const validateName = (rule, value, callback) => {
      ...
      if( value.replace(/\s+/g,"") == '' ){
        ...
        callback(new Error('标签内容长度不超过20'))
      }else{
        callback()
      }
    };
    return {
      ...
      rules: {
        labelContent: [{ required: true,validator:validateName}],
        englishName: [{ required: false},{max:40,message:'英文名称不超过40'}],
      },
      dialogStatus: '',
      loading: false,
  ...
  methods: {
    ...
    handleUpdate(row) {//行内修改
      ...
      this.temp = {id, labelContent , englishName, sort, parentId, labelId}
      this.dialogFormVisible = true;
      this.$nextTick(() => {
        this.$refs['dataForm'].clearValidate()
      })
