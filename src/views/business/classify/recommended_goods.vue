    ...
    selectSubmit(){
      if (this.goodsSelection.length>0) {
        ...
        addAPI(this.temp).then(res => {
          operationMsg.apply(this,[res.data,`添加${this.goodsSelection.length}项商品`])
          this.sending = false
    ...
    setTop(type){
      if(this.multipleSelection.length>0){
        ...
        topStatusAPI(this.massQuery).then(res => {
          type===0 ? operationMsg.apply(this,[res.data,`取消置顶${len}项`]) : operationMsg.apply(this,[res.data.code,`置顶${len}项`])
        })
      } else {
