  ...
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="handleSelectionChange" ref="multipleTable" >
      ...
      <el-table-column label="订单号" align="center">
        <template slot-scope="scope">
          <router-link class="bookLink" v-if="true" :to="{path:'order/order_processing',query:{id:scope.row.orderId}}">{{scope.row.orderCode}}</router-link>
        </template>
      </el-table-column>
...
export default {
  data() {
    return {
      ...
      checkId:'',
      timeRange: [],
      isBatchAuditBtn:true,
      isExportExcelBtn:true,
      isCancelBtn:true,
      isConfirmBtn:true,
      isReviewConfirmBtn:true,
      isStoragesBtn:true,
      isChoiceWareBtn:true,
...
created() {
  ...
  methods: {
    batchAudit(type) {
      if(this.isSelected()&&this.isBatchAuditBtn){
        this.isBatchAuditBtn=false;
        if(type == 1){
    ...
    storages(){
      if(this.isStoragesBtn){
        this.isStoragesBtn=false;
        let nameStorages='';
