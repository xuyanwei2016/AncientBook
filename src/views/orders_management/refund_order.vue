<template>
  <div class="app-container">
    <div class="buttons">
      ...
      <!-- <el-button icon="el-icon-document" @click="batchCancel">批量取消退款</el-button> -->
      <el-button v-if="authBtns.map(i => i.idName).indexOf('batchCancelbtn') >= 0" @click="batchCancel">
        {{authBtns.filter(i => i.idName=='batchCancelbtn')[0].name}}<!--<i v-if="authBtns.filter(i => i.idName=='batchCancelbtn').length>0 && authBtns.filter(i => i.idName=='batchCancelbtn')[0].icon" :class="[authBtns[authBtns.map(i => i.idName).indexOf('batchCancelbtn')].icon, 'el-icon&#45;&#45;right']"></i>-->
      </el-button>
    </div>
    ...
    <el-form inline class="form-inline">
      ...
      <!-- <template v-if="moreFilter" > -->
        <el-form-item label="订单号">
          <el-input  placeholder="订单号、退货单号、退款单号" v-model="listQuery.orderCode" clearable style="width:230px;">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="handleSelectionChange" ref="multipleTable" >
      ...
      <el-table-column label="退货单号" align="center">
        <template slot-scope="scope">
          <router-link :to="{ path:`refund_goods/goods_processing`, query:{id:scope.row.salesReturnId}}">{{scope.row.salesReturnCode}}</router-link>
        </template>
      </el-table-column>
      <el-table-column label="原订单号" align="center">
        <template slot-scope="scope">
          <router-link :to="{ path:`order/order_processing`, query:{id:scope.row.orderId}}">{{scope.row.orderCode}}</router-link>
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="small-padding fixed-width operationBtns" label="操作" align="center">
        <template slot-scope="scope">
          <ul style="text-align:center">
            ...
            <li v-if="authBtns.map(i => i.idName).indexOf('refundbtn') >= 0 && (scope.row.status ==1)"><i @click="refund(scope.row.id)">{{authBtns.filter(i => i.idName=='refundbtn')[0].name}}</i></li>
...
export default {
  data() {
    return {
      ...
      isRefundBtn:true,
      isCheckBtn:true,
      isBatchCheckBtn:true,
      isBatchConfirmBtn:true,
      isBatchCancelBtn:true,
      isDealOrderBtn:true,
...
created() {
  ...
  mounted() {
    ...
    if( ifUndefined(store.getters.refund_orderSerKeys.pageNum) ){
      ...
      if(ifUndefined(this.listQuery.beginCreateTime) || ifUndefined(this.listQuery.endCreateTime)){
        this.timeRange = []
        this.timeRange[0] = this.listQuery.beginCreateTime
        this.timeRange[1] = this.listQuery.endCreateTime
      }
    }
    this.getList()
  ...
  methods: {
    ...
    batchCheck() {//批量审核通过
      if(this.isSelected()&&this.isBatchCheckBtn){
        this.isBatchCheckBtn=false;
    ...
    batchConfirm() {//批量退款
      if(this.isSelected()&&this.isBatchConfirmBtn){
        this.isBatchConfirmBtn=false;
    ...
    batchCancel(){//批量取消退款
      if(this.isSelected()&&this.isBatchCancelBtn){
        this.isBatchCancelBtn=false;
        confirm.apply(this,['确定要取消批量退款吗？']).then(() => {
          batchCancelAPI(this.batchCancelMsg).then(res => {
            if(res.data.code == -1 ){
              this.$message.success(res.data.msg)
            }else if(res.data.code == 0){
              if(res.data.data.length>0){
                this.$message.warning(`请单独操作订单${res.data.data}查看原因`)
              }else{
                this.$message.success("批量取消退款成功")
              }
            }else{
              this.$message.error("批量取消退款失败")
            }
            this.getList()
            this.batchCancelMsg=[];
            this.isBatchCancelBtn=true;
          })
        }).catch(req=>{
          this.isBatchCancelBtn=true;
        })
    ...
    dealOrder(id,code){//详情展示
      if(this.isDealOrderBtn){
        this.isDealOrderBtn=false;
        this.orderInfoId = id
    ...
    handleFilter() {
      if(this.isHandleFilterBtn){
        ...
        if (this.timeRange) {
          ...
          this.listQuery.endCreateTime = this.timeRange[1]
        } else {
