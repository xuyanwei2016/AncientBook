<template>
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="listData" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe>
      ...
      <el-table-column label="运送方式" align="center">
        <template slot-scope="scope">
          <span>{{postageType[scope.row.expressType]}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="small-padding fixed-width" label="操作" width="200" align="center">
        <template slot-scope="scope">
          <router-link :to="{ path:`settle/add_settle`, query:{editId:scope.row.id}}" v-if="authBtns.filter(i => i.idName=='editbtn').length > 0"><i :class="authBtns.filter(i => i.idName=='editbtn')[0].icon" :title="authBtns.filter(i => i.idName=='editbtn')[0].name" ></i></router-link>
...
<script>
  import { getListAPI, deleteAPI } from '@/api/postage/settle'
