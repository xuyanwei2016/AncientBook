    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="chioceList" v-show="!pushing">
      ...
      <el-table-column label="入库单号" min-width="130" align="center">
        <template slot-scope="scope">
          <router-link :to="{path:`storage/details_storage`, query:{bookId:scope.row.id}}">{{scope.row.code}}</router-link>
        </template>
      </el-table-column>
      <el-table-column label="关联单号" min-width="120" align="center">
        <template slot-scope="scope">
          <span @click="linkBtn(scope.row.type,scope.row.relationId)">
            <!--<router-link :to="{path:'transfer/details_transfer',query:{detailsId:scope.row.id}}">-->{{scope.row.relationCode}}<!--</router-link>-->
          </span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    linkBtn(type,id){
      let linkObj={
        0:'purchase/details_purchase',
        3:'inventory/details_inventory',
        4:'/orders_management/refund_goods/goods_processing',
        8:'transfer/details_transfer'
      };
      let linkId={
        0:{path: linkObj[type], query:{bookId:id,pageType:1}},
        3:{path: linkObj[type], query:{detailsId:id}},
        4:{path: linkObj[type], query:{id:id}},
        8:{path: linkObj[type], query:{bookId:id}}
      };
      /*判断是不是含有英文字符*/
      /*let haveEN=/[a-z]/.test(id);//包含true 没有事false*/
      this.$router.push(linkId[type])
     //0 采购入库  3 增益入库  4退货入库  8调拨入库
    },
  }
}
