  ...
  <div class="app-container" v-loading="pushing"  element-loading-text="正在推送...">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="chioceList"  v-show="!pushing">
      ...
      <el-table-column label="关联单号" align="center">
        <template slot-scope="scope">
          <span @click="linkBtn(scope.row.type,scope.row.relationId)">{{scope.row.relationCode}}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      batchSubmitOrder() {//批量提交
        if(this.newList.length == 0){
          ...
          if(status == true){
            ...
            this.$message.warning('只有未提交的数据才能提交')
          }
        }
      },
      batchPushOrder() {//批量推送-百世
        if(this.newList.length == 0){
          ...
          if(status == true){
            ...
            this.$message.warning('推送数据必须为推送失败或者未确认的百世库且不能重复提交')
          }
      ...
      linkBtn(type,id){
        let linkObj={
          0:'/orders_management/pick_order/pick_processing',
          3:'inventory/details_inventory',
          7:'returnpurchase/details_returnpurchase',
          8:'transfer/details_transfer'
        };
        let linkId={
          0:{path: linkObj[type], query:{id:id}},
          3:{path: linkObj[type], query:{detailsId:id}},
          7:{path: linkObj[type], query:{id:id}},
          8:{path: linkObj[type], query:{bookId:id}}
        };
        this.$router.push(linkId[type])
        //0 销售出库  3 报损出库  7退书出库  8调拨出库
      },
    }
  }
