<template>
  <div class="app-container">
    <el-form label-position="left" inline class="demo-table-expand">
      <el-form-item label="采购单退货号" label-width="100px">
        <span>{{ note.rcode}}</span>
      </el-form-item>
    </el-form>
    ...
    <el-table :data="bookList" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe>
      ...
      <el-table-column prop="date" label="退货价" align="center">
        <template slot-scope="scope">
          <span>{{scope.row.returnPrice}}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import { detailsListAPI } from '@/api/stock-management/returnpurchase'
  export default{
    directives: { waves },
    data(){
      return {
    ...
    methods:{
      getList() {
        ...
        const oldId = this.$route.query.id;
