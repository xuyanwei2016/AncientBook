  ...
  <div class="app-container">
    ...
    <el-form inline class="form-inline">
      ...
      <el-form-item label="出库单号">
        <el-input v-model="listQuery.code" placeholder="请输入出库单号" clearable></el-input>
      </el-form-item>
    ...
    <el-table :key='tableKey' :data="listData" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe>
      ...
      <el-table-column label="出库单号" align="center">
        <template slot-scope="scope">
          <router-link :to="{path:'delivery/details_delivery',query:{bookId:scope.row.id}}">{{scope.row.code}}</router-link>
        </template>
      </el-table-column>
...
<script>
  ...
  import { getListAPI, getWareAPI, getWarePlace, deliveryTypeAPI, getDeliveryStatusAPI, getpublisherAPI, getNavAPI, getOptionAPI, getCountAPI } from '@/api/stock-management/delivery-book'
  ...
  export default {
    ...
    mounted() {
      if( ifUndefined(store.getters.deliveryBookSerKeys.pageNum) ){
        this.listQuery = store.getters.deliveryBookSerKeys
    ...
    methods: {
      ...
      downLoadFile() {//导出
        ...
        paramStr ? window.open(`${process.env.BASE_API}${requestPath.stock}/delivery_book/export?${paramStr}`) : window.open(`${process.env.BASE_API}${requestPath.stock}/delivery_book/export`)
      },
      handleFilter() {
        if(this.timeLimit){
        ...
        store.dispatch('SetDeliveryBookSerKeys',this.listQuery).then(res => {
