  ...
  <div class="app-container xyw">
    ...
    <el-form inline class="form-inline" :model="listQuery">
      ...
      <el-form-item label="注册时间">
        <el-date-picker value-format="yyyy-MM-dd" format="yyyy-MM-dd" v-model="time" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
        </el-date-picker>
      </el-form-item>
      <el-form-item>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="handleSelectionChange" @current-change="handleCurChange" ref="multipleTable">
      ...
      <el-table-column width="280px" align="center" label="注册时间">
        <template slot-scope="scope">
          <span>{{scope.row.beginDate}}</span><!-- | parseTime('{y}-{m}-{d} {h}:{i}') -->
        </template>
      </el-table-column>
      <el-table-column width="280px" align="center" label="到期时间">
        <template slot-scope="scope">
          <span>{{scope.row.endDate}}</span> <!-- | parseTime('{y}-{m}-{d} {h}:{i}') -->
        </template>
      </el-table-column>
      ...
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" disabled label-width="100px" style='margin-left:50px;' width="400">
        ...
        <el-form-item label="会员类型" prop="type">
          <el-select placeholder="正式" v-model="temp.type">
            <el-option v-for="adType in adTypes" :key="adType.value" :label="adType.label" :value="adType.value">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="注册时间" prop="beginDate">
          <el-col :span="11">
            <el-date-picker type="date" placeholder="选择日期" v-model="temp.registeTime" style="width: 100%;"></el-date-picker>
          </el-col>
        </el-form-item>
  ...
  export default {
    data() {
      ...
      return {
        ...
        adTypes: [{value: 1, label: "正式"}, {value: 2, label: "试用"}],
        temp: {
          account: "",
          organName: "1111",
          type: '',
    ...
    mounted() {      
      if( ifUndefined(store.getters.orgMemberSerKeys.pageNum) ){
        this.listQuery = store.getters.orgMemberSerKeys
        console.log(this.listQuery,'读取搜索条件成功')
        if(ifUndefined(this.listQuery.beginDate) || ifUndefined(this.listQuery.endDate)){
          this.time = []
          this.time[0] = this.listQuery.beginDate
          this.time[1] = this.listQuery.endDate
        }
      }
    ...
    methods: {
      ...
      massSettingSubmit() {
        batchUpdateStastusAPI(this.memberStatus).then(res => {
          operationMsg.apply(this,[res.data,`批量设置${this.multipleSelection.length}项`])
        ...
        if (this.multipleSelection.length != 0) {
          this.$confirm("确定初始化用户密码吗？", "提示", {
            ...
            batchResetCodeAPI(this.ids).then(res => {
              operationMsg.apply(this, [res.data, "重置"])
            })
          });
        } else {
          this.$message.warning("请先选择需要设置的内容")
        ...
        this.$refs["dataForm"].validate(valid => {
          if (valid) {
            ...
            getListAPI(tempData).then(res => {
              operationMsg.apply(this, [res.data, '广告位更新'])
