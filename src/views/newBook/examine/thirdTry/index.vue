            ...
            >
              ...
              <el-table-column label="操作" align="center" width="120px">
                <template slot-scope="scope">
                  ...
                  <i class="copy distance" title="审核" v-if="scope.row.logStatus==0"  @click="looktoRecord(scope.row,'editId')"></i>
...
<script>
  ...
  import {getSubListAPI,batchAuditAPI} from '@/api/newBook/examine.js';
  export default {
    name: "thirdTry",
    data() {
      return {
    ...
    methods: {
      ...
      looktoRecord(item,name){
        this.$router.push({path:`/newBook/examine/examineView?lookId=${item.id}${name=='editId'?'&type=3':''}&logStatus=${item.logStatus}&status=2-3-3`});
      },
      /*勾选列表*/
