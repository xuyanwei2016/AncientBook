      ...
      >
        ...
        <div class="tabwrapper1">
          ...
          <el-form-item prop="productSize" label="成品尺寸：" class="width360">
            <el-input v-model="imgForm.productSize" name="productSize" :disabled="vueStatus"></el-input>
            mm
          </el-form-item>
          <el-form-item prop="wordCount" label="字数：" class="width360">
            <el-input v-model="imgForm.wordCount" name="wordCount" :disabled="vueStatus"></el-input>
            千字
          </el-form-item>
          <el-form-item prop="pages" label="页数：" class="width360">
            <el-input v-model="imgForm.pages" name="pages" :disabled="vueStatus"></el-input>
            页
          </el-form-item>
          <el-form-item prop="illustrations" label="插图：" class="width360">
            <el-input v-model="imgForm.illustrations" name="illustrations" align="left"
                      :disabled="vueStatus"></el-input>
            副
          </el-form-item>
        ...
        <div class="tabwrapper1">
          ...
          <el-form-item prop="sameAsLastTime" label="是否按上个版次印刷：" class="width360  labelLine">
            <el-radio-group v-model="imgForm.sameAsLastTime" :disabled="vueStatus">
              <el-radio label="1">是</el-radio>
              <el-radio label="0">否</el-radio>
            </el-radio-group>
          </el-form-item>

          <el-form-item prop="modelBookSituation" label="样书情况：">
            <el-input v-model="imgForm.modelBookSituation" name="modelBookSituation" type="textarea"
                      :rows="4" style="width: 80%" resize="none" :disabled="vueStatus"></el-input>
          </el-form-item>
          <el-form-item prop="modelBookRemark" label="备注：">
            <el-input v-model="imgForm.modelBookRemark" name="modelBookRemark" type="textarea" :rows="4" style="width: 80%" resize="none"
                      :disabled="vueStatus"></el-input>
          </el-form-item>
          ...
          <el-form-item prop="times" label="时间：" class="width360">
            <el-date-picker
              v-model="imgForm.times" type="date" placeholder="选择日期"
              value-format="yyyy-MM-dd" :disabled="vueStatus">
            </el-date-picker>
          </el-form-item>
          <el-form-item prop="remark" label="备注：">
            <el-input v-model="imgForm.remark" name="remark" type="textarea" :rows="4" style="width: 80%"
                      resize="none" :disabled="vueStatus"></el-input>
          </el-form-item>
        ...
        <div  v-if="((vueStatus&&step1Data.length>0)||($route.query.examineId))&&((Number(imgForm.status)>0)||(Number(imgForm.status)==0&&imgForm.stage=='7-3'))">
          ...
          <div class="tabwrapper1">
            ...
            <div class="steps">
              <ul>
                <li v-for="(step,inds) in steps" :key="inds" @click="curStep>=inds&&StepFn(step)"
                    :class="curStep>=inds?'active':''">{{step.name}}
                </li>
              </ul>
            </div>
          </div>
...
<script>
  ...
  import stepOne from './overprint/step1.vue';
  import stepTwo from './overprint/step2.vue';
  ...
  export default {
    data() {
      return {
        ...
        rules:{
          ...
          sameAsLastTime:[{required: true, message: '请选择是否按上个版次印刷', trigger: 'change'}],
          modelBookSituation:[{required: true, message: '请输入样书情况', trigger: 'change'},{max: 100, message: '样书情况长度小于100', trigger: 'change'}],
          modelBookRemark:[{required: true, message: '请输入备注', trigger: 'change'},{max: 100, message: '备注长度小于100', trigger: 'change'}],
        ...
        isSeries: false,
        vueStatus: false,
        steps: [{name: '图书加印', fatherStage: '7-4'}, {name: '图书入库', fatherStage: '7-3'}],
        curStep:null,
        fatherStage:'7-4',
        isStep:'7-4',
        step1Data:[],
    ...
    created() {
      if (this.$route.fullPath.indexOf('editId') >= 0) {
        this.$route.meta.title = '编辑图书加印信息';
      } else if (this.$route.fullPath.indexOf('lookId') >= 0) {
        this.$route.meta.title = '查看图书加印信息';
        this.vueStatus = true;
      } else {
        this.$route.meta.title = '审核图书加印信息';
        this.vueStatus = true;
      }
      this.getDetail();
        ...
        getDetailAPI(this.$route.query.editId || this.$route.query.lookId || this.$route.query.examineId).then(res => {
          if (res.data.status) {
            ...
            if(this.$route.query.examineId) {
              this.curStep = this.steps.findIndex((item) => {
                return this.imgForm.stage == item.fatherStage;
              })
            }
          ...
          if(!this.$route.query.examineId){
            for(let i=0;i<2;i++){
              if(i==0&&this.step1Data.length>0){
                ...
                this.curStep=0;
              }else if(i==1&&this.step2Data.styleBookCount&&this.step2Data.styleBookCount!=''){
                this.curStep=1;
              }
            }
