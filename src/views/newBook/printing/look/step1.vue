  ...
  <div class="addImgResource newBookView xywCss">
    <div class="formbox1">
      ...
      ><!--:rules="rules"-->
        ...
        <!--总编室主任意见-->
        <div class="tabwrapper1" v-if="dataList.length<5&&!vueStatus">
        ...
        <div class="tabwrapper2" v-if="dataList.length<5&&!vueStatus">
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        vueStatus: false,
        stage:[
          {title:'责编意见',stage:'7-1-2-1',name:'责任编辑',remark:'责编建议印数'},
          {title:'总编室主任意见',stage:'7-1-2-2',name:'总编室主任',remark:'是否通过再质检及发排时费用是否财务确认过'},
          {title:'财务意见',stage:'7-1-2-3',name:'财务',remark:'财务费用情况(加印图书必填)'},
          {title:'发行部意见',stage:'7-1-2-4',name:'发行部主任',remark:'发行部建议印数'},
          {title:'社领导意见',stage:'7-1-2-5',name:'社领导',remark:'社领导意见'},
        ],
      }
    },
    created() {
    ...
    methods: {
      ...
      dataSubmit(){
        this.$refs['step1Form'].validate((valid) => {
          if(valid){
            this.getTime();
            this.step1Form.bookType = this.$route.name=='secondEdition'?'2':'1';
            this.step1Form.fatherStage = '7-1';
