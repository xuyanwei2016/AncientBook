    ...
    <div class="formbox">
      ...
      >
        ...
        <div class="tabwrapper1">
          ...
          <el-form-item prop="wordCount" label="字数（千字）：" class="width360">
        ...
        <div class="tabwrapper1">
          ...
          <el-form-item prop="bindingOrder" label="装订顺序：">
            <el-input v-model="imgForm.bindingOrder" name="bindingOrder" type="textarea" :rows="4" maxlength="200" style="width: 80%"  resize="none" :disabled="vueStatus"></el-input>
          </el-form-item>
          <el-form-item prop="otherRequirements" label="其他要求：">
            <el-input v-model="imgForm.otherRequirements" name="otherRequirements" type="textarea" :rows="4" maxlength="200" style="width: 80%"  resize="none" :disabled="vueStatus"></el-input>
          </el-form-item>
          <el-form-item prop="deliveryStatus" label="包销（送货）情况：">
            <el-input v-model="imgForm.deliveryStatus" name="deliveryStatus" type="textarea" :rows="4" maxlength="200" style="width: 80%"  resize="none" :disabled="vueStatus"></el-input>
          </el-form-item>
          ...
          <el-form-item prop="remark" label="备注：">
            <el-input v-model="imgForm.remark" name="remark" type="textarea"
                      :rows="4" maxlength="200" style="width: 80%" resize="none" :disabled="vueStatus"></el-input>
          </el-form-item>
        ...
        <div v-if="((vueStatus&&step1Data.length>0)||($route.query.examineId))&&((Number(imgForm.status)>0)||(Number(imgForm.status)==0&&imgForm.stage=='7-3'))">
          <!--印制流程-->
          <!-- v-if="(imgForm.status!=0&&imgForm.status!=1)||(imgForm.status==1&&!$route.query.lookId)||(imgForm.status==0&&imgForm.stage=='7-3')"-->
        ...
        <!--<div v-if="vueStatus&&(step3Data&&step3Data.styleBookCount!='')">
          &lt;!&ndash;印制流程&ndash;&gt;
          <div class="tabwrapper1" v-if="(imgForm.status!=0&&imgForm.status!=1)||(imgForm.status==1&&!$route.query.lookId)||(imgForm.status==0&&imgForm.stage=='7-3')">
            <h6>&nbsp;&nbsp;|&nbsp;&nbsp;印制流程</h6>
            <div class="steps">
              <ul>
                <li v-for="(step,inds) in steps" :key="inds" @click="curStep>inds&&StepFn(step)" :class="curStep>inds?'active':''">{{step.name}}</li>
  ...
  export default {
    data() {
      return {
        ...
        rules:{
          ...
          framing:[{required: true, message: '请输入装帧', trigger: 'change'},{max: 10, message: '装帧输入长度小于10', trigger: 'change'}],
        ...
        isShowStep:false,
        isStep:'7-1',
        steps:[{name:'新书付型',fatherStage:'7-1'},{name:'审核毛书',fatherStage:'7-2'},{name:'图书入库',fatherStage:'7-3'}],
        stage: {
          '7-1': '新书付型',
          '7-2': '毛书',
          '7-3': '图书入库',
        ...
        step1Data:[],/*新书付型 审核日记*/
        step2Data:[],/*审核毛书 审核日记*/
        step3Data:{styleBookCount:''},/*图书入库 审核日记*/
        curStep:null,
      }
    },
    ...
    created(){
      if (this.$route.fullPath.indexOf('editId') >= 0) {
        ...
        this.$route.meta.title = '查看图书印刷信息';
        this.vueStatus = true;
      } else {
        this.$route.meta.title = '审核图书印刷信息';
        this.vueStatus = true;
      }
      this.getDetail();
    },
      ...
      getDetail(){
        getDetailAPI(this.$route.query.editId||this.$route.query.lookId||this.$route.query.examineId).then(res=>{
          if(res.data.status){
            ...
            if(this.$route.fullPath.indexOf('editId')<0&&this.$route.fullPath.indexOf('lookId')<0){
              this.$route.meta.title = `审核${this.stage[this.imgForm.stage]}信息`;
            }
