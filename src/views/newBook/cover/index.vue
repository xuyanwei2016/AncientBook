      ...
      <div class="tablelist">
        <template>
          <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick">
            ...
            <el-tab-pane label="录入中" name="6-1"></el-tab-pane>
            <el-tab-pane label="待审核" name="6-2"></el-tab-pane>
            <el-tab-pane label="审核通过" name="6-3"></el-tab-pane>
            <el-tab-pane label="设计完成" name="6-5"></el-tab-pane>
          </el-tabs>
        </template>
          ...
          >
            ...
            <el-table-column label="状态" align="center">
              <template slot-scope="scope">
                <span v-if="scope.row.stage  =='6-1'">录入中</span>
                <span v-if="scope.row.stage  =='6-2'">待审核</span>
                <span v-if="scope.row.stage  =='6-3'">审核通过</span>
                <span v-if="scope.row.stage  =='6-5'">设计完成</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" align="center">
              <template slot-scope="scope">
                ...
                <i class="edit distance" title="编辑" v-if="scope.row.stage  !='6-3'" @click="looktoRecord(scope.row,'editId')"></i>

                <i class="imdel" title="删除" @click="deleteOne(scope.row)" v-if="scope.row.stage == '6-1'&&!scope.row.subjectNum" ></i>
...
import pagination from "../../../views/resource/components/pagination";
  ...
  import {getListAPI,submitReviewAPI,batchDeteleAPI} from '@/api/newBook/cover.js';
export default {
  name: "subjectIndex",
  components: {
    pagination
  ...
  methods: {
    ...
    looktoRecord(item,name) {
      this.$router.push({path:`/newBook/cover/view?${name}=${item.id}`});
    },
    //提交审核
    addOne() {
      this.$router.push('/newBook/cover/view');
    },
    ...
    changeStatus(num){
      let self=this;
      let data=[
        {stage:'6-1',status:'录入中',curStage:'6-2'},
        {stage:'6-2',status:'待审核',curStage:'6-3'},
        {stage:'6-3',status:'审核通过',curStage:'6-5'},
      ];
      ...
      let stampV = this.batchList.every((item) => {
        return item.printedColors !=''  && item.printedColors!=null;
      })
    ...
    deleteOne(item){
      let ids=[item.id];
      confirm.apply(this,['确定要删除选择的图书封面设计信息吗？']).then(() => {
