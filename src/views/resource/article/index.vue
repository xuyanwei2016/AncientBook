<template>
  <el-container class="project-plan">
    ...
    <el-main>
      <div class="buttons">
        <router-link :to="`article/add_article?type=add`">
      ...
      <div class="formlist">
        <el-form ref="form" :model="querylist" label-width="82px">
          ...
          <el-form-item label="数据来源" v-show="hideFlag">
      ...
      <div class="tablelist">
        <template>
          <el-tabs v-model="activeName" type="border-card" @tab-click="statusChange">
            <el-tab-pane label="所有文章" name="first"></el-tab-pane>
        ...
        <div class="table_outer_wrap">
          ...
          >
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                <router-link :to="{path: `article/add_article`, query: {id: scope.row.id}}">
                  <i
                    class="edit distance"
                    title="编辑"
                ...
                <router-link :to="{path: `article/add_article?id=${scope.row.id}&type=view`}">
                  <i
                ...
                <router-link :to="`article/relatedResource?id=${scope.row.id}&name=${scope.row.name}`">
                  <i
                    class="link distance"
                    title="关联资源"
  ...
  methods: {
    ...
    confirmUpdatePrice() {
      this.$refs['priceForm'].validate( valid =>{
        if(valid){
         if (isNaN(this.updatePriceForm.sellingPrice) == true) {
