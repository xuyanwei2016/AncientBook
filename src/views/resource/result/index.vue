<template>
    ...
    <el-container class="project-plan">
        ...
        <el-aside width="199px"  class="tree-aside">
            <el-tree :data="navTree" :highlight-current='true' :props="defaultProps" @node-click="handleNodeClick" default-expand-all></el-tree>
        </el-aside>
        ...
        <el-main>
            ...
            <router-link :to="`result`">
              <el-button type="primary" @click="importPicture" v-if="authBtns.filter(i => i.idName=='importBtn').length>0">
                <i class="imBook"></i>导入成果
              </el-button>
              <!-- <el-button type="primary" >
                <i class="imExcel"></i>导入成果
              </el-button> -->
            </router-link>
            <router-link :to="`result/add_result`" >
              <el-button type="primary" v-if="authBtns.filter(i => i.idName=='addBtn').length>0">
                  <router-link :to="`result/add_result?type=add`">
                  <i class="add"></i>添加</router-link>
              </el-button>
            </router-link>
            </div>
            ...
            <div class="formlist">
                <el-form ref="form" :model="querylist" label-width="100px">
                    <el-form-item label="成果名称:" class="width360">
                        <el-input @keyup.enter.native="onSubmit" v-model="listQuery.name"></el-input>
                    </el-form-item>
                    <el-form-item label="成果完成人:">
                        <el-input @keyup.enter.native="onSubmit" v-model="listQuery.author"></el-input>
                    </el-form-item>
                    <el-form-item label="第一完成单位:" class='width460'>
                        <el-input @keyup.enter.native="onSubmit" v-model="listQuery.completeOrgan "></el-input>
                    </el-form-item>
           ...
           <div class="tablelist">
               <template >
                    <el-tabs v-model="activeName" type="border-card" @tab-click="statusChange">
                        <el-tab-pane label="所有成果" name="first"></el-tab-pane>
                ...
                <el-table height="450"  border v-loading="listLoading" element-loading-text="给我一点时间" ref="multipleTable1" :data="list" tooltip-effect="dark" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column label="成果完成人" align="center">
                        <template slot-scope="scope">
                            {{scope.row.author}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column align="center" label="操作">
                        <template slot-scope="scope">
                            <router-link :to="{path: `result/add_result`, query: {id: scope.row.id}}" >
                              <i class="edit distance" title="编辑" v-if="authBtns.filter(i => i.idName=='editBtn').length>0"></i>
                            </router-link>
                            <!-- <router-link :to="`article/add_article?id={scope.row.id}&type=update`"> <i class="edit distance"></i></router-link> -->
                            <router-link :to="{path: `result/add_result`, query: {detailsId: scope.row.id}}">
                              <i class="search distance" title="查看" v-if="authBtns.filter(i => i.idName=='viewBtn').length>0"></i>
                            </router-link>
                            <router-link :to="`result/relatedResource?id=${scope.row.id}`">
...
import { getListAPI,batchingAuditAPI,batchingDelAPI,getTreeAPI,updateTypeAPI,toLeadAPI} from '@/api/resource/result'
