        ...
        <el-tab-pane label="资源信息" name="first">
          <div class="tabwrapper">
            ...
            >
              ...
              <el-form-item prop="bookName" class="width360">
                <span slot="label">所属图书:</span>
                <el-input v-model="imgForm.bookName" :disabled="this.dialogStatus === 'view'" data-vv-as="field" name="owner"></el-input>
              </el-form-item>
              <el-form-item prop="source" class="width360">
                <span slot="label">来源:</span>
                <el-input v-model="imgForm.source" :disabled="this.dialogStatus === 'view'" data-vv-as="field" name="from"></el-input>
              </el-form-item>
              ...
              <el-form-item prop="picList" class="width100">
                <span slot="label">
                  动画封面:
                </span>
                <el-upload
              ...
              <el-form-item label-width="150px">
                ...
                <p class="alert-info">支持AVI、WMV、RMVB、MP4、MOV、FLV、FLA等格式，不超过2G</p>
                <p class="animation-error" v-if="animationError">请上传动画文件</p>
                <el-upload
                  class="upload-demo"
                  ...
                  :disabled="this.dialogStatus === 'view'"
                >
                  <el-button size="small" type="primary" :loading="loadAnimation" v-if='hiddenViewButton'>点击上传</el-button>
                </el-upload>
                <video v-if="animationUrl" :src="animationUrl" controls class="video"></video>
              </el-form-item>
              <h6>&nbsp;&nbsp;|&nbsp;&nbsp;商品信息</h6>
              ...
              <el-form-item prop="salesStatus">
                <span slot="label">销售状态:</span>
                <el-radio-group v-model="imgForm.salesStatus">
                  <el-radio :label="1">可售</el-radio>
                  <el-radio :label="2">不可售</el-radio>
                </el-radio-group>
              </el-form-item>
              <el-form-item label="定价:" prop="price" class="width325">
                <el-input v-model="imgForm.price" :disabled="this.dialogStatus === 'view'"></el-input>
                <span>元</span>
                <span class="recomment">在前台展示为划线价格</span>
              </el-form-item>
              <el-form-item label="售价:" prop="priceType">
                <span slot="label">售价:</span>
...
import { files, getDepartmentAPI,getCopyrightListAPI2,fileListAPI,sectionAPI} from "@/api/resource/common";
...
export default {
  ...
  data() { 
    return {
      ...
      rules: {
        ...
        author: { max: 10, message: "长度10字以内"},
        bookName: [{ max: 80, message: "长度在80字以内"}],
        source: [{ max: 30, message: "长度在30字以内"}],
        departmentId: { required: true, message: "请输入所属部门"},
        goosCode: [{ max: 20, messgae: "长度20字以内"}],
        dataSources: { required: true, message: "请输入数据来源" },
        salesStatus: { required: true, message: "请输入销售状态"},
        price: {pattern: /^\d{0,5}(.\d{0,2})?$/,message: "请输入0-99999之间的数字，最多保留两位"},
        sellingPrice: {required: true,pattern: /^(\d{0,5})(\.\d{0,2})?$/,message: "请输入0-99999之间的数字，最多保留两位"},
        contractNumber: { max: 20, message: "长度20字以内"},
        contractName: { max: 50, message: "长度50字以内"},
        diyTypeId:[{required:true,message:'请填写自定义分类'}],
        keywords:[{required:false,max:100,message:"关键字的长度不超过100"}]
      },
      canUseDate: false,
  ...
  methods: {
    ...
    beforeVideoUpload(file) {
      ...
      if (isVideo!='mp4'&&isVideo!='avi'&&isVideo!='rmvb'&&isVideo!='wmv'&&isVideo!='mov'&&isVideo!='flv'&&isVideo!='fla') {
        this.$message.error("视频只支持AVI、WMV、RMVB、MP4、MOV、FLV、FLA!");
        return false;
      }
  ...
  created() {
    ...
    this.$route.meta.title = this.changeTitle() + "动画";
  },
  mounted() {
    //this.getNav();
    this.$route.meta.title = "添加动画";
    if (this.dialogStatus === "view" || this.dialogStatus === "update") {
      if (this.dialogStatus === "view") {
        this.$route.meta.title = "查看动画";
      } else {
        this.$route.meta.title = "修改动画";
      }
      this.getInfo();
    }
    this.getTree();
