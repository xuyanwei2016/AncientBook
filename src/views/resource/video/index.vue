  ...
  <el-container class="project-plan">
    ...
    <el-main>
      <div class="buttons">
        ...
        >
          <i class="imBook"></i>导入视频
        </el-button>
        <router-link :to="`video_store/add_video?type=add`">
      ...
      <div class="tablelist">
        <template>
          <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick">
            <el-tab-pane label="所有视频" name="first"></el-tab-pane>
        ...
        <div class="table_outer_wrap">
          ...
          >
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                <router-link :to="`video_store/add_video?id=${scope.row.id}&type=update`">
                  <i
                    class="edit distance"
                    title="编辑"
                ...
                <router-link :to="`video_store/add_video?id=${scope.row.id}&type=view`">
                  <i
                ...
                <router-link :to="`video_store/relatedResource?id=${scope.row.id}&name=${scope.row.name}`">
                  <i
                    class="link distance"
                    title="关联资源"
    ...
    <!--导入视频对话框-->
    <el-dialog title="导入视频" @close="closeImport"  :visible.sync="importDialog" class="middle backgroundDialog dialog460">
      ...
      <el-form :model="importForm" label-width="82px" :rules="rules" style="margin-top:20px">
      ...
      <div>
        <div class="dialogbtns dialog-footer">
          <el-button type="primary" @click="importAll" :loading="loadingVideo">全部导入</el-button>
...
export default {
  ...
  methods: {
    ...
    handleSuccess(res, file, fileList) {
      ...
      if(this.acount==this.$refs['uploadDom'].uploadFiles.length){//文件上传完毕时
        if (this.importForm.names.length > 0) {
          ...
          importVideoAPI(this.importForm).then(res => {
    ...
    closeImport(){
      this.importDialog = false;
      this.loadingVideo = false;//按钮的加载效果
      this.fileList = [];
      this.acount=0;
      this.importForm = {
        diyTypeCode: "",
    ...
    handleClick(tab, event) {
      this.currentPageFun(1);//切换tab初始化分页当前页为1;
      if (tab.index == "0") {
