      ...
      <el-tabs v-model="activeName" @tab-click="handleClick" type="border-card">
        <el-tab-pane label="资源信息" name="first">
          <div class="tabwrapper">
            ...
            >
              ...
              <el-form-item prop="bookName" class="width360" label="所属图书:">
                <el-input
                  v-model="imgForm.bookName"
                  data-vv-as="field"
                  name="bookName"
                  :disabled="this.dialogStatus === 'view'"
                ></el-input>
              </el-form-item>
              <el-form-item prop="source" class="width360" label="来源:">
                <el-input
              ...
              <el-form-item prop="departmentId" class="width360" label="所属部门:">
                <el-select
              ...
              <el-form-item prop="cover" class="width100" label="音频封面:">
                <el-upload
              ...
              <el-form-item label-width="150px" prop="audio">
                <span slot="label">音频文件:</span>
                <el-upload
                  class="upload-demo"
                ...
                <audio v-if="audioUrl" :src="audioUrl" controls class="audio" style='margin-top:20px;'></audio>
                <span>{{imgForm.audioName}}</span>
              </el-form-item>
              <h6>&nbsp;&nbsp;|&nbsp;&nbsp;商品信息</h6>
        ...
        <el-dialog title="关联合同" :visible.sync="relatedDialog" class="backgroundDialog relatedDialog">
          <el-form :model="relatedForm" label-width="120" style='margin-bottom:5px;' class='relatedFormInput'>
            ...
            <el-button type="primary" @click="handleFilter" @icon="el-icon-search">检索</el-button>
          </el-form>
...
import { files, getDepartmentAPI,getCopyrightListAPI,fileListAPI,sectionAPI } from "@/api/resource/common";
...
export default {
  ...
  data() {
    return {
      ...
      uploadFileList: {
        ...
        token: "",
        oid: this.$route.query.id || this.$route.query.add_audio,
      },
      ...
      rules:{
        ...
        bookName: [{max:80, message:"所属图书长度不可以超过80"}],
        source:[{required:true,message:'请输入来源'},{max:30,message:"来源长度不可以超过30"}],
        dataSources:[{required:true,message:'请输入数据来源'}],
        audio:[{required:true,message:"请上传音频文件"}],
        keywords:[{max:100,message:"关键词长度不可以超过100"}],
        ...
        // selfClass:[{ required: true,message:"自定义分类必填",trigger:'blur'}],
        goosCode:[{max:20,message:"商品编码长度不可以超过20"}],
        salesStatus: [{ required: true, message: "请选择销售状态" }],
        diyTypeId:[{required:true,message:'请选择分类',trigger:'input'}]
      },
      noDiy:false,
      //自定义标签
      labelList:[],
  ...
  methods: {
    ...
    //操作记录文件列表
    getFileList() {
      console.log(this.fileForm,"pfdf")
    ...
    beforeaudioUpload(file) {
      ...
      if (!isaudio) {
        this.$message.error("音频只支持MP3");
        return isaudio;
      }
      if (!isLt2G) {
        this.$message.error("上传音频大小不能超过 2G!");
        return isLt2G;
      }
      this.$message.warning('上传中...')
    },
    handleaudioSuccess(res, file, fileList) {
      this.audioUrl = `${this.domain}${this.requestPath.file}?fileName=${
        res.fileName
      }&isOnLine=true`;
      this.imgForm.audio = res.fileName;
      this.imgForm.audioName=file.name
      this.imgForm.size=res.size
      this.imgForm.timeLength=res.time
      this.audioFile = true;
      this.$refs['ruleForm'].clearValidate('audio')
    },
    getInfo() {
      detailsAPI(this.$route.query.id).then(res => {
        if (res.data.code === 0) {
         ...
         if(this.imgForm.audio){
           this.audioUrl= `${this.domain}${this.requestPath.file}?fileName=${
              this.imgForm.audio
              }&isOnLine=true`;
         }
         console.log(this.audioUrl,"pppp")
    ...
    isEdit() {
      this.$route.meta.title = "添加音频";
      if (this.$route.query.type == 'update') {

        this.$route.meta.title = "修改音频";
        this.getInfo();
      }
      if (this.$route.query.type == 'view') {
        this.pageStatus = 1;
        this.$route.meta.title = "查看音频";
        this.getInfo();
      }
    },
  },
