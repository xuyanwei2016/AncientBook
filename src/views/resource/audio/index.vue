  ...
  <el-container class="project-plan">
    ...
    <el-main>
      <div class="buttons">
        ...
        >
          <i class="imBook"></i>导入音频
        </el-button>
        <router-link :to="`audio_store/add_audio?id=''&type=add`">
      ...
      <div class="tablelist">
        <template>
          <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick">
            <el-tab-pane label="所有音频" name="first"></el-tab-pane>
        ...
        <div class="table_outer_wrap">
          ...
          >
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                <router-link :to="`audio_store/add_audio?id=${scope.row.id}&type=update`">
                  <i
                    class="edit distance"
                    title="编辑"
                ...
                <router-link :to="`audio_store/add_audio?id=${scope.row.id}&type=view`">
                  <i
                ...
                <router-link
                  :to="`audio_store/relatedResource?id=${scope.row.id}&name=${scope.row.name}`"
                >
                  <i
                    class="link distance"
                    title="关联资源"
    ...
    <!--导入音频对话框--> 
    <el-dialog title="导入音频" :visible.sync="importDialog" @close="closeImport" class="middle backgroundDialog dialog460">
...
export default {
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      const isPDF = file.type == "audio/mp3"||"audio/mpeg";
    ...
    handleSuccess(res, file, fileList) {
      ...
      if(this.acount==this.$refs['uploadDom'].uploadFiles.length){//文件上传完毕时
        ...
        if (this.importForm.names.length > 0) {
          ...
          importAudioAPI(this.importForm).then(res => {
