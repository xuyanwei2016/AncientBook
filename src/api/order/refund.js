...
export function batchCancelAPI(data) { //批量取消退款
    return request({
        url: `${requestPath.order}/refund-order/batch/update/cancel`,
        method: 'post',
        data
    })
}
export function onlyCancelAPI(data) { //单独取消退款
    return request({
        url: `${requestPath.order}/refund-order/update/${data.oid}/cancel`,
        method: 'post',
        data
    })
}
