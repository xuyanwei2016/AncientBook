import { loginByUsername, logout, getUserInfo, login, getRouterList, getPageButtonList } from '@/api/login'
  ...
  actions: {
    Login({ commit }, userInfo) {
      ...
      return new Promise((resolve, reject) => {
        login(username, userInfo.password,userInfo.captcha,userInfo.uuid).then(response => {
          ...
          Cookies.set('userInfo',response.data.data.userName);
    ...
    GetPageButtonList({ commit, state }, pageCode) {
      return new Promise((resolve, reject) => {
        getPageButtonList(pageCode, state.token).then(res => {
          // console.log('调用了获取按钮的接口',res.data.data)
    ...
    GetRouterList({ commit, state }) {
      return new Promise((resolve, reject) => {
        ...
        getRouterList({ token: state.token }).then(res => {
          resolve(res)
        }).catch(error => {
          reject(error)
        })
